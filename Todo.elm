...
Because todomvc uses ports, it won't work when looking at Todo.elm directly in elm-reactor.  You will need to look at index.html instead after the elm code has been compiled (which will happen either when you try to load Todo.elm via elm-reactor, or when you run `elm-make Todo.elm` from the command line).
